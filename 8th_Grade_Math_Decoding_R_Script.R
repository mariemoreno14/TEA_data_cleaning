...
# Keep columns that match this pattern: "District Number" OR  start with "District YEAR" AND "Grade 3" AND end with "STAAR Reading/ELA Rate" 
# Check column names
cols_to_keep_2013 <- c("District Number", grep("^District STAAR.*Grade 8.*Mathematics Rate", colnames(data_2013), value = TRUE))
data_2013 <- data_2013[, cols_to_keep_2013]
colnames(data_2013)

cols_to_keep_2014 <- c("District Number", grep("^District 2014.*Grade 8.*STAAR Mathematics Rate", colnames(data_2014), value = TRUE))
data_2014 <- data_2014[, cols_to_keep_2014]
colnames(data_2014)

cols_to_keep_2016 <- c("District Number", grep("^District 2016.*Grade 8.*STAAR Mathematics Rate", colnames(data_2016), value = TRUE))
data_2016 <- data_2016[, cols_to_keep_2016]
colnames(data_2016)

cols_to_keep_2017 <- c("District Number", grep("^District 2017.*Grade 8.*STAAR Mathematics Rate", colnames(data_2017), value = TRUE))
data_2017 <- data_2017[, cols_to_keep_2017]
colnames(data_2017)

cols_to_keep_2018 <- c("District Number", grep("^District 2018.*Grade 8.*STAAR Mathematics Rate", colnames(data_2018), value = TRUE))
data_2018 <- data_2018[, cols_to_keep_2018]
colnames(data_2018)

cols_to_keep_2019 <- c("District Number", grep("^District 2019.*Grade 8.*STAAR Mathematics Rate", colnames(data_2019), value = TRUE))
data_2019 <- data_2019[, cols_to_keep_2019]
colnames(data_2019)

cols_to_keep_2021 <- c("District Number", grep("^District 2021.*Grade 8.*STAAR Mathematics Rate", colnames(data_2021), value = TRUE))
data_2021 <- data_2021[, cols_to_keep_2021]
colnames(data_2021)

cols_to_keep_2022 <- c("District Number", grep("^DISTRICT 2022.*Grade 8.*STAAR Mathematics Rate", colnames(data_2022), value = TRUE))
...
# extract the year and group identifier from each column name
...
new_col_names_2013 <- gsub(".* (\\d{4}) (.+?) Mathematics.*", "\\1: \\2", col_names_2013)
new_col_names_2014 <-  gsub(".* (\\d{4}) .*, Grade 8, (.+), STAAR.*", "\\1: \\2", col_names_2014)
new_col_names_2016 <- gsub(".* (\\d{4}) .*, Grade 8, (.+), STAAR.*", "\\1: \\2", col_names_2016)
new_col_names_2017 <- gsub(".* (\\d{4}) .*, Grade 8, (.+), STAAR.*", "\\1: \\2", col_names_2017)
new_col_names_2018 <- gsub(".* (\\d{4}) .*: (Masters|Approaches|Meets) .* (\\d), (.+?),.*", "\\1: \\2: \\4", col_names_2018)
new_col_names_2019 <- gsub(".* (\\d{4}) .*: (Masters|Approaches|Meets) .* (\\d), (.+?),.*", "\\1: \\2: \\4", col_names_2019)
new_col_names_2021 <- gsub(".* (\\d{4}) .*: (Masters|Approaches|Meets) .* (\\d), (.+?),.*", "\\1: \\2: \\4", col_names_2021)
new_col_names_2022 <- gsub(".* (\\d{4}) .*: (Masters|Approaches|Meets) .* (\\d), (.+?),.*", "\\1: \\2: \\4", col_names_2022)
...
data_reshaped_2018 <- data_2018 %>%
  pivot_longer(cols = -`District Number`,
               names_to = "Year_Group",
               values_to = "Rate") %>%
  separate(Year_Group, into = c("Year", "Outcome", "Group"), sep = ": ", remove = FALSE)
...
data_reshaped_2013 <- data_reshaped_2013[, c("District Number", "Year", "Group", "Rate")]
data_reshaped_2014 <- data_reshaped_2014[, c("District Number", "Year", "Group", "Rate")]
data_reshaped_2016 <- data_reshaped_2016[, c("District Number", "Year", "Group", "Rate")]
data_reshaped_2017 <- data_reshaped_2017[, c("District Number", "Year", "Group", "Rate")]
data_reshaped_2018 <- data_reshaped_2018[, c("District Number", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2019 <- data_reshaped_2019[, c("District Number", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2021 <- data_reshaped_2021[, c("District Number", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2022 <- data_reshaped_2022[, c("District Number", "Year", "Outcome", "Group", "Rate")]
...
# Convert Rate column values to decimals
...
data_reshaped_2022 <- data_reshaped_2022 %>%
  mutate(Rate = Rate/100)
# Dataframe contains masked data...those rates will become -0.010
...
data_reshaped_2013 <- data_reshaped_2013[, c("District Name", "Year", "Group", "Rate")]
data_reshaped_2014 <- data_reshaped_2014[, c("District Name", "Year", "Group", "Rate")]
data_reshaped_2016 <- data_reshaped_2016[, c("District Name", "Year", "Group", "Rate")]
data_reshaped_2017 <- data_reshaped_2017[, c("District Name", "Year", "Group", "Rate")]
data_reshaped_2018 <- data_reshaped_2018[, c("District Name", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2019 <- data_reshaped_2019[, c("District Name", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2021 <- data_reshaped_2021[, c("District Name", "Year", "Outcome", "Group", "Rate")]
data_reshaped_2022 <- data_reshaped_2022[, c("District Name", "Year", "Outcome", "Group", "Rate")]

# Format District Name column to have "Title" case capitalization--first letter capitalized 
# Also ensure rows ending in "Isd" are capitalized to "ISD"
data_reshaped_2013$"District Name" <- str_to_title(data_reshaped_2013$"District Name")
data_reshaped_2013$"District Name" <- ifelse(str_detect(data_reshaped_2013$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2013$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2013$"District Name")
data_reshaped_2014$"District Name" <- str_to_title(data_reshaped_2014$"District Name")
data_reshaped_2014$"District Name" <- ifelse(str_detect(data_reshaped_2014$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2014$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2014$"District Name")
data_reshaped_2016$"District Name" <- str_to_title(data_reshaped_2016$"District Name")
data_reshaped_2016$"District Name" <- ifelse(str_detect(data_reshaped_2016$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2016$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2016$"District Name")
data_reshaped_2017$"District Name" <- str_to_title(data_reshaped_2017$"District Name")
data_reshaped_2017$"District Name" <- ifelse(str_detect(data_reshaped_2017$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2017$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2017$"District Name")
data_reshaped_2018$"District Name" <- str_to_title(data_reshaped_2018$"District Name")
data_reshaped_2018$"District Name" <- ifelse(str_detect(data_reshaped_2018$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2018$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2018$"District Name")
data_reshaped_2019$"District Name" <- str_to_title(data_reshaped_2019$"District Name")
data_reshaped_2019$"District Name" <- ifelse(str_detect(data_reshaped_2019$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2019$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2019$"District Name")
data_reshaped_2021$"District Name" <- str_to_title(data_reshaped_2021$"District Name")
data_reshaped_2021$"District Name" <- ifelse(str_detect(data_reshaped_2021$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2021$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2021$"District Name")
data_reshaped_2022$"District Name" <- str_to_title(data_reshaped_2022$"District Name")
data_reshaped_2022$"District Name" <- ifelse(str_detect(data_reshaped_2022$"District Name", "Isd$"), 
                                             str_replace(data_reshaped_2022$"District Name", "Isd$", "ISD"), 
                                             data_reshaped_2022$"District Name")
...
# Stack dataframes and fill null values for the extra column
stacked_data <- bind_rows(data_reshaped_2013, data_reshaped_2014, data_reshaped_2016, data_reshaped_2017, data_reshaped_2018, data_reshaped_2019, data_reshaped_2021, data_reshaped_2022)

# mutate() with recode() function to replace specific values in the "Group" column
...
# Check values for Group and District Name
#sort(unique(stacked_data$Group))
#sort(unique(stacked_data$'District Name'))

# Remove rows with "approaches" or "masters" as outcome as we only want NA (which denotes a pass) or "meets"
stacked_data <- stacked_data %>%
  filter(!Outcome %in% c("Approaches", "Masters"))

# remove rows where the district name is missing or NA
stacked_data <- stacked_data[complete.cases(stacked_data$'District Name'), ]

# Final review
View(stacked_data)
 
# write the updated data to a new excel file--type file path to Tableau Ready folder!!
write_xlsx(stacked_data, "/Users/heidimariemoreno/portfolio/8th_Grade_Math_2013-2022/RScript_Output/District_Complete.xlsx")
